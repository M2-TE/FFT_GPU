	...
	//for iteration3:
		...
		SB[ind2+k] = sign0*SA[ind2]*SROT[2*r6+k] + SA[ind2+1]*SROT[2*r6+(!k)];
		__syncthreads();
	//for iteration4:
		...
		SA[ind3+k+x] = sign3*(SB[index7+k] + sign1*SB[ind3+k]);
		SB[ind3+k] = SA[index8+k] + sign2*SA[ind3+k];
