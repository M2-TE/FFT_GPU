...
template<uint N, bool bDebug = false> inline void perform_custom_fft(FFTData<float>& data)
{
	...
	else fft<N> KERNEL_GRID(gridDim, blockDim)(data.deviceData, data.deviceData);
}