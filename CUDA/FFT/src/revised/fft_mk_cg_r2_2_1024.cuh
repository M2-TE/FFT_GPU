...
#define M N/2
#define D N/4
__global__ void kernel0FFT(float* A, float* ROT)
{
...
__global__ void kernel1FFT(float* A, float* ROT)
{
	__shared__ float SA[M], SB[M], SROT[2 * N];
	short j = threadIdx.x;
	short b = blockIdx.x;
	...
	//stage6:
	s = 6;
	short r4 = (1 << (s - 2)) * (((j % 2) << 1) + ((j >> 1) % 2)) * ((j % (1 << (n - 2))) >> (s - 2));
	SB[ind2] = SA[ind0] + signk * SA[ind1];
	SB[ind2 + 1] = SA[ind0 + 1] + signk * SA[ind1 + 1];
	__syncthreads();
	SA[ind2] = SB[ind2] * SROT[r4 << 1] + SB[ind2 + 1] * SROT[(r4 << 1) + 1];
	SA[ind2 + 1] = -SB[ind2] * SROT[(r4 << 1) + 1] + SB[ind2 + 1] * SROT[r4 << 1];
	__syncthreads();
	...
	//stage8:
	s = 8;
	short r5 = (1 << (s - 2)) * (((j % 2) << 1) + ((j >> 1) % 2)) * ((j % (1 << (n - 2))) >> (s - 2));
	SA[ind2] = SB[ind0] + signk * SB[ind1];
	SA[ind2 + 1] = SB[ind0 + 1] + signk * SB[ind1 + 1];
	__syncthreads();
...
void DoA()
{
	...
	kernel0FFT << <gridDim, blockDim >> > (Ad, ROTd);
...
cudaMemcpy(ROT, ROTd, rotsize,  cudaMemcpyDeviceToHost);
	...
	kernel1FFT << <gridDim, blockDim >> > (Ad, ROTd);
