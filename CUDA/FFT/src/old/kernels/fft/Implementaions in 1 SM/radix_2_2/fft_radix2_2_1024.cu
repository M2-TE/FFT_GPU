	...
	short ind0,ind1,ind2,ind3,ind4,r1,r4,r6,r8,sign1,sign2,index1,index2,index3,index4,index5,index6,index7,index8,index9,index10;
	short i = j/2;
	short k = j%2;
	short h = i/2;
	short l = j%4;
	...
	//for iteration3:
		...
		s0 = 6;
		r6 = (1<<(s0-2))*((((ind2>>(n1-s0+1))%2)<<1)+((ind2>>(n1-s0+2)))%2)*((ind2>>1)%(1<<(n1-s0)));
		index6 = ind2+sign2*p2;
		
		SA[ind2] = SB[index6] + sign2*SB[ind2];
		SA[ind2+1] = SB[index6+1] + sign2*SB[ind2+1];

		SB[ind2] = SA[ind2]*SROT[2*r6] + SA[ind2+1]*SROT[2*r6+1];
		SB[ind2+1] = -SA[ind2]*SROT[2*r6+1] + SA[ind2+1]*SROT[2*r6];
		__syncthreads();
	//for iteration4:
		short s3=4;
		short p3 = M/(1<<(2*s3));
		short ind_tmp3 = 2*(h+(h/(1<<2*(n-s3)))*(1<<2*(n-s3))*3);
				
		ind3 = ind_tmp3+l*p3;
		//stage1:
		index7 = ind3+sign1*2*p3;
				
		SA[ind3+w] = signw*(SB[index7] + sign1*SB[ind3]);
		SA[ind3+!w] = SB[index7+1] + sign1*SB[ind3+1];
		
		//stage2:
	 

		s0 = 8;
		r8 = (1<<(s0-2))*((((ind3>>(n1-s0+1))%2)<<1)+((ind3>>(n1-s0+2)))%2)*((ind3>>1)%(1<<(n1-s0)));
		index8 = ind3+sign2*p3;
		
		SB[ind3] = SA[index8] + sign2*SA[ind3];
		SB[ind3+1] = SA[index8+1] + sign2*SA[ind3+1];

		SA[ind3] = SB[ind3]*SROT[2*r8] + SB[ind3+1]*SROT[2*r8+1];
		SA[ind3+1] = -SB[ind3]*SROT[2*r8+1] + SB[ind3+1]*SROT[2*r8];
		__syncthreads();
	//for iteration5:
		
		short s4 = 5;
		short p4 = M/(1<<(2*s4));
		short ind_tmp4 = 2*(h+(h/(1<<2*(n-s4)))*(1<<2*(n-s4))*3);
				
		ind4 = ind_tmp4+l*p4;
		
		//for stage1:	
		index9 = ind4+sign1*2*p4;

		//for stage2:
		index10 = ind4+sign2*p4;
		
		SB[ind4+w]    = signw*(SA[index9] + sign1*SA[ind4]);
		SB[ind4+!w]   = SA[index9+1] + sign1*SA[ind4+1];
		SA[ind4]      = SB[index10] + sign2*SB[ind4];
		SA[ind4+1]    = SB[index10+1] + sign2*SB[ind4+1];
