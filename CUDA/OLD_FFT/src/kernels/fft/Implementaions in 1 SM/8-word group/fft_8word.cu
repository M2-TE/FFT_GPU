...
__global__  void fft(float* A, float* ROT)
{
	...
	for (unsigned int i = 0; i < 8; i++) SROT[tid + i * blockDim.x] = ROT[tid + i * blockDim.x];

	// transfer samples from global to shared memory
	for (unsigned int i = 0; i < 16; i++) SA[tid + i * blockDim.x] = A[tid + i * blockDim.x];
	...
	for (short s = 1; s <= n; s++)
	{
		...
		r5 = r4 + (N / 4); // special rotation
		r6 = r4; // trivial rotation
		r7 = r5; // special rotation
		...
		// TODO: forgo use of shared memory entirely outside of initial read and final write!
...
#pragma unroll
	for (unsigned int i = 0; i < 16; i++) A[tid + i * blockDim.x] = SA[tid + i * blockDim.x];
}
